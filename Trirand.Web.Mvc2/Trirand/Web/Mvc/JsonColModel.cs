namespace Trirand.Web.Mvc
{
    ...
    internal class JsonColModel
    {
        ...
        private void ApplyFormatterOptions(JQGridColumn column)
        {
            ...
            if (column.Formatter != null)
            {
                ...
                if (formatter is CustomFormatter)
                {
                    ...
                    if (!string.IsNullOrEmpty(formatter7.SetAttributesFunction))
                    {
                        this._jsonValues["cellattr"] = formatter7.SetAttributesFunction;
                    }
                }
            }
        ...
        public void FromColumn(JQGridColumn column)
        {
            ...
            if (column.Editable)
            {
                ...
                if (column.EditList.Count<SelectListItem>() > 0)
                {
                    ...
                    int num2 = 0;
                    foreach (SelectListItem item2 in column.EditList)
                    {
        ...
        private ArrayList GetSearchOptionsArray(List<SearchOperation> options)
        {
            ...
            foreach (SearchOperation operation in options)
            {
                list.Add(this.GetStringFromSearchOperation(operation));
            }
            return list;
        }
            ...
            foreach (JQGridColumn column in grid.Columns)
            {
                if (column.Formatter != null)
                {
                    ...
                    if (formatter is CustomFormatter)
                    {
                        ...
                        str = str.Replace(oldValue, newValue);
                        oldValue = string.Format("\"cellattr\":\"{0}\"", formatter2.SetAttributesFunction);
                        newValue = string.Format("\"cellattr\":{0}", formatter2.SetAttributesFunction);
                        str = str.Replace(oldValue, newValue);
                    }
                }
                foreach (JQGridEditClientSideValidator validator in column.EditClientSideValidators)
