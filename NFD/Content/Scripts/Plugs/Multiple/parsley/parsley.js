...
$(function () {
    ...
    /// <reference path="../../../../imgs/vtip_arrow.png" />
    ...
    $("[reg],[url]:not([reg]),[tip]").hover(
		function (e) {
		    if ($(this).attr('tip') != undefined) {
		        ...
		        $('body').append('<p id="vtip"><img id="vtipArrow" src="/Content/imgs/vtip_arrow.png"/>' + $(this).attr('tip') + '</p>');
...
$.fn.easyValidate = function (pisAlert) {
    var errorInfo = "";
    var thObj = this;
    execIf(pisAlert);
    function execIf(isAlert) {
        thObj.each(function (i, v) {
            var th = $(this);
            var thVal = $(this).val();
            var tip = th.attr("tip");
            var reg = new RegExp(th.attr("reg"), "gi");
            if (reg.test(thVal.toString())) {
                th.removeClass("input_validation-failed")
            } else {
                errorInfo += tip + "\n";
                th.addClass("input_validation-failed")
            }

        });
        if (isAlert && errorInfo != "") {
            alert(errorInfo);
            thObj.val("");
        }
    }
    //    $("[reg]").die();
    //    $("[reg]").live("blur", function () {
    //        execIf(false);
    //    });
    return errorInfo == "";
}
